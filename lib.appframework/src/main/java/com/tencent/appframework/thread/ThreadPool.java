...
public class ThreadPool {
    ...
    public ThreadPool() {
        this("common-pool", CORE_POOL_SIZE, MAX_POOL_SIZE);
    }
     ...
     * 负责消息回调处理
     */
    private static volatile Handler MESSAGE_THREAD_HANDLER;

    /**
     * 发送消息和定时器等轻量级的用，其它的不要用啊。切记
     */
    private static volatile Handler SEND_MSG_HANDLER;
    private static volatile HandlerThread SEND_MSG_THREAD;
    ...
    /**
     * 获得副线程的Handler. 副线程可以执行比较快但不能在ui线程执行的操作.
     *
     * @return handler
     */
    ...
    /**
     * 返回一个"线性"的Executor. <br> 通过此对象的execute()方法执行的任务会串行执行. 但不会新建线程, 而是使用线程池中的线程, 因此仍然会受到线程池的限制.<br>
     *
